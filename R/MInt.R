...
#' @import MASS
#' @export
estimate <- function(mfit, lambda=NULL) {
  ...
  if (!mfit$param$ready) mfit <- initialize_parameters(mfit, lambda);
...
initialize_parameters <- function(mfit, lambda) {
  # Initialize the parameters.
  # beta
  # Start with a log-normal fit. 
  mfit$param$xb <- matrix(NA, nrow=nrow(mfit$data$y), ncol=ncol(mfit$data$y));
